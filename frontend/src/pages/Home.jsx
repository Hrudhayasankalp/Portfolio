      ...
      <section className="min-h-[80vh] flex flex-col justify-center relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <p className="text-gray-400 text-base md:text-xl max-w-lg mx-auto md:mx-0 leading-relaxed pt-2 md:pt-4">
      ...
      {skills.length > 0 && (
        <section id="skills" className="py-12 border-t border-gray-800/50">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-400 px-4">Tools and technologies I work with</p>
          </div>
          <div className="flex flex-wrap justify-center gap-3 md:gap-6 px-4">
            {skills.map((skill, index) => (
              <motion.div
                ...
                className="px-5 md:px-8 py-3 md:py-4 glass rounded-xl md:rounded-2xl text-base md:text-lg font-medium hover:bg-white/10 hover:border-white/20 transition-colors cursor-default shadow-lg"
              >
                {skill.name}
              </motion.div>
            ))}
            ...
            <div className="relative border-l border-gray-700 ml-4 space-y-12">
              {experience.map((exp, index) => (
                ...
                >
                  ...
                  <div className="glass-card p-6 md:p-8 rounded-2xl hover:border-blue-500/30 transition-colors group">
                    <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-base md:text-lg text-gray-300 font-medium">{exp.company}</p>
                      </div>
                      <span className="inline-flex items-center px-4 py-1.5 rounded-full text-xs md:text-sm font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20 w-fit whitespace-nowrap">
                        {exp.startDate} - {exp.endDate || "Present"}
                      </span>
                    </div>

                    <p className="text-sm md:text-base text-gray-400 leading-relaxed mb-6 whitespace-pre-wrap">{exp.description}</p>
          ...
          <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 px-4">
            {education.map((edu, index) => (
              <motion.div
                ...
                className="glass-card p-6 md:p-8 rounded-2xl hover:border-purple-500/30 transition-colors group"
              >
                <div className="flex justify-between items-start mb-4">
                  <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-purple-500/10 text-purple-400 flex items-center justify-center text-xl md:text-2xl border border-purple-500/20">
                    🎓
                  </div>
                  <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] md:text-xs font-medium bg-purple-500/10 text-purple-400 border border-purple-500/20">
                    {edu.startYear} - {edu.endYear || "Present"}
                  </span>
                </div>

                <h3 className="text-lg md:text-xl font-bold text-white group-hover:text-purple-400 transition-colors">{edu.institution}</h3>
                <p className="text-sm md:text-base text-gray-300 font-medium mt-1">{edu.degree} in {edu.field}</p>
                {edu.grade && (
                  <p className="text-xs md:text-sm text-gray-400 mt-3 pt-3 border-t border-gray-700/50">
